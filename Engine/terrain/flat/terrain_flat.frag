...
in float pass_variationBlend;
in float pass_heightBlend;
in float pass_mountainBlend;
...
void main(void) {
    ...
    if (uIsPlanetary > 0.5) {
        ...
        variationBlend = pass_variationBlend;
        heightBlend = pass_heightBlend; 
        mountainBlend = pass_mountainBlend;
        ...
        // Use interpolated procedural noise from vertex shader
        variationBlend = pass_variationBlend;
        heightBlend = pass_heightBlend;
        mountainBlend = pass_mountainBlend;
        worldRef = worldXZ;
            ...
            if (R > 0.0 && alpha > 0.01) {
                ...
                if (t > 0.0) {
                    ...
                    if (distSq < rSq) {
                        ...
                        // OPTIMIZATION: Removed noise from cloud shadows to prevent 32x noise calls per pixel
                        float perturbedDist = dist;
    ...
    if (projCoords.x >= 0.0 && projCoords.x <= 1.0 && 
        ...
        // OPTIMIZATION: 4-tap PCF filter (FPS boost)
        vec2 offsets[4] = vec2[](
            vec2(-1.0, -1.0), vec2(1.0, -1.0),
            vec2(-1.0, 1.0), vec2(1.0, 1.0)
        );
        for (int j = 0; j < 4; j++) {
            float pcfDepth = texture(shadowMap, projCoords.xy + offsets[j] * texelSize).r;
            totalLight += (currentDepth - bias) > pcfDepth ? 0.0 : 1.0;
        }
        globalShadow = totalLight / 4.0;
    } else {
        globalShadow = 1.0;
    }
