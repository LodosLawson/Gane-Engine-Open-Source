...
out float pass_variationBlend;
out float pass_heightBlend;
out float pass_mountainBlend;
...
void main(void) {
    ...
    // --- OPTIMIZATION: Calculate macro noise in Vertex Shader instead of Fragment Shader ---
    vec2 worldXZ = worldPos.xz;
    mat2 rot = mat2(0.8, -0.6, 0.6, 0.8);
    float macroNoise = noise(rot * worldXZ * 0.002);
    pass_variationBlend = smoothstep(0.35, 0.65, macroNoise);
    
    float shoreNoise = (noise(rot * worldXZ * 0.05) * 8.0 - 4.0) + (noise(worldXZ * 0.2) * 2.0 - 1.0);
    pass_heightBlend = smoothstep(2.0, 16.0, worldPos.y + shoreNoise);
    
    float mountainNoise = (noise(rot * worldXZ * 0.015) * 16.0 - 8.0) + (noise(worldXZ * 0.08) * 4.0 - 2.0);
    pass_mountainBlend = smoothstep(40.0, 95.0, worldPos.y + mountainNoise);
